...
- **台词保持原始输入（绝对禁止翻译）**：台词内容**严禁翻译为任何其他语言**，必须将 videoDesc 中的台词原文逐字照抄输出。例如：videoDesc 中台词为中文，输出必须为中文；为英文则输出英文。违反此规则视为严重错误。
...
1. **Instruction 必须用英文**（台词除外，台词原文照抄，绝对不翻译）
...
4. **台词不可缺失且绝对不翻译**：videoDesc 中有台词的分镜，必须在 Instruction 中将台词**原文逐字照抄**，严禁翻译为英文或任何其他语言
5. **台词类型标注**：
   - 普通对白 → `(dialogue)`
